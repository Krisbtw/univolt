...
function EmergencyView({ p, onClose }: { p: EmgPayload; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex flex-col gap-4 overflow-y-auto bg-red-950 p-6">
      <button onClick={onClose} className="self-start text-sm font-medium text-red-200">← Back</button>
      <div>
      ...
      <div className="flex flex-col gap-3 rounded-[16px] border border-red-600/40 bg-red-900/60 p-4">
        <EmgField label="Case ID"     value={p.id} />
        <EmgField label="Blood Group" value={p.bg ?? "Not recorded"} />
        <EmgField label="Allergies"   value={p.alg ?? "None on file"} />
        <EmgField label="Conditions"  value={p.cond ?? "None on file"} />
        <EmgField label="Medication"  value={p.med ?? "None on file"} />
      </div>
      {(p.bpm != null || p.rr != null || p.spo2 != null) && (
        <div className="rounded-[16px] border border-red-600/40 bg-red-900/60 p-4">
...
function EmgField({ label, value }: { label: string; value: string }) {
  return (
    <div>
...
function BigStat({ label, value, unit }: { label: string; value: string; unit: string }) {
  return (
    <div className="text-center">
      <p className="text-[10px] font-semibold uppercase text-red-400">{label}</p>
  ...
  const startScan = useCallback(async () => {
    ...
    try {
      ...
      scanIntervalRef.current = setInterval(async () => {
        ...
        } catch { /* per-frame errors are expected */ }
      }, 500);
  ...
  return (
    <AppFrame>
      ...
      <main className="flex flex-1 flex-col gap-4 px-4 pb-10 pt-3">
        ...
        <div id="emg-card" className="rounded-[20px] border border-red-400/30 bg-paper p-5">
          ...
          {editMode ? (
            <div className="flex flex-col gap-3">
              ...
              {([
                { fid: "input-allergies",  label: "Allergies",          val: allergies,   set: setAllergies },
                { fid: "input-conditions", label: "Conditions",         val: conditions,  set: setConditions },
                { fid: "input-medication", label: "Current Medication", val: medication,  set: setMedication },
              ] as const).map(({ fid, label, val, set }) => (
                <div key={fid}>
            ...
            <div className="flex flex-col gap-2">
              <InfoRow label="Blood Group" value={bloodGroup === "unknown" ? "Unknown" : bloodGroup} />
          ...
          <div className="mt-4 border-t border-line pt-3">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <VitalBox label="HR"   value={lastCam    ? `${lastCam.heartRate}`        : "--"} unit={lastCam    ? "bpm" : ""} />
              <VitalBox label="RR"   value={lastCam    ? `${lastCam.respiratoryRate}`  : "--"} unit={lastCam    ? "/min" : ""} />
              <VitalBox label="SpO₂" value={lastManual ? `${lastManual.spo2Estimate}`  : "--"} unit={lastManual ? "%"   : ""} />
            </div>
            {!lastManual && (
              <p className="mt-1 text-[10px] text-faint">SpO₂ not measured — enter manual reading on patient page.</p>
            )}
          </div>
        ...
        {/* Scan mode */}
        {scanMode && (
          <div className="flex flex-col gap-3 rounded-[16px] border border-line bg-paper p-4">
            <p className="text-sm text-ink">Point camera at a printed emergency card QR code.</p>
            {scanError === "unsupported"   && <p className="text-sm text-amber-400">QR scanning is not supported. Use your camera app.</p>}
            {scanError === "camera_denied" && <p className="text-sm text-red-400">Camera access denied.</p>}
